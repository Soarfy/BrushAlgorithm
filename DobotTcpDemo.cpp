...
void DobotTcpDemo::RelMovJDemo( Dobot::CDescartesPoint& pt,int user,int tool,int a,int v,int cp){
    std::string result;
    {
        std::lock_guard<std::mutex> lk(m_mutexDashboard);
        result = m_Dashboard.RelMovJTool(pt, user, tool, a, v, cp);
    }
    if (!waitQueuedCommandFinish(result, "RelMovJTool", 30000)) {
        std::cout << "RelMovJTool wait timeout or failed, reply: " << result << std::endl;
    }
}
                                 ...
                                 double& rx, double& ry, double& rz)
{
    ...
    {
        std::lock_guard<std::mutex> lk(m_mutexDashboard);
        recv = m_Dashboard.GetPose(user, tool);
    }
...
void DobotTcpDemo::startDrag(){
    std::lock_guard<std::mutex> lk(m_mutexDashboard);
    m_Dashboard.EnableRobot();
    std::string result = m_Dashboard.StartDrag();
    std::cout << "StartDrag: " << result << std::endl;
}

void DobotTcpDemo::stopDrag(){
    std::lock_guard<std::mutex> lk(m_mutexDashboard);
    std::string result = m_Dashboard.StopDrag();
    std::cout << "StopDrag: " << result << std::endl;
}
    ...
    const Dobot::CDescartesPoint& pointb)
{
    std::string result;
    {
        std::lock_guard<std::mutex> lk(m_mutexDashboard);
        m_Dashboard.EnableRobot();
        result = m_Dashboard.MovJ(pointa);
    }
    if (!waitQueuedCommandFinish(result, "MovJ", 30000)) {
        std::cout << "MovJ wait timeout or failed, reply: " << result << std::endl;
    }
...
bool DobotTcpDemo::waitQueuedCommandFinish(const std::string& recvData, const char* commandName, int timeoutMs)
{
    ...
    getCurrentCommandID(recvData, currentCommandID);
    if (currentCommandID == 2147483647) {
        std::cout << commandName << " no valid command id, skip wait. recv: " << recvData << std::endl;
        return false;
    }
    return waitCommandIdFinish(currentCommandID, timeoutMs);
}

bool DobotTcpDemo::waitCommandIdFinish(int currentCommandID, int timeoutMs)
{
    ...
    while (true) {
        {
            ...
            if (feedbackData.ErrorStatus) {
                ...
                {
                    std::lock_guard<std::mutex> lk(m_mutexDashboard);
                    errIdRaw = m_Dashboard.GetErrorID();
                }
                std::vector<std::string> errorIdVec = regexRecv(errIdRaw);

...
int main()
{
    ...
    points = {
        ...
        // // 右边牙刷位置
        // {155.428,458.358,360.167,-134.557,-0.261,90.539},
        // {25.411, 452.19, 422.686, -179.534, -0.215, 90.558},
        // // 左侧的
        // {-110.597,458.595,360.209,136.188,-0.285,90.538},
        // {25.411, 452.19, 422.686, -179.534, -0.215, 90.558},
    };
    ...
    for (const auto &pt : points)
    {
        ...
        else if (count == 17)
        {
            ...
            // {
                ...
                // c2::MovJointSegments definetrajright;
                // c2::Point cpos1right;
                // [155.428,458.358,360.167,-134.557,-0.261,90.539]
                // cpos1right.type = c2::PointType::Cart;
                // cpos1right.cpos.x = 168.324;
                // cpos1right.cpos.y = 452.202;
                // cpos1right.cpos.z = 382.428;
                // cpos1right.cpos.a = -134.536;
                // cpos1right.cpos.b = -0.215;
                // cpos1right.cpos.c = 90.558;
                // definetrajright.AddMovJ(cpos1right, speed, acc, c2::ZoneType::Fine, zone);
                // arm.StartBrushTeethTrajectoryJoint(definetrajright);
                // 提示用户安装新的牙刷
            //     char answers = 'n';
            //     while (answers != 'y')
            //     {
            ...
            // {
                ...
                // c2::MovJointSegments definetrajleft;
                // c2::Point cpos1left;
                // [-110.597,458.595,360.209,136.188,-0.285,90.538]
                // cpos1left.type = c2::PointType::Cart;
                // cpos1left.cpos.x = 168.324;
                // cpos1left.cpos.y = 452.202;
                // cpos1left.cpos.z = 382.428;
                // cpos1left.cpos.a = -134.536;
                // cpos1left.cpos.b = -0.215;
                // cpos1left.cpos.c = 90.558;
                // definetrajleft.AddMovJ(cpos1left, speed, acc, c2::ZoneType::Fine, zone);
                // arm.StartBrushTeethTrajectoryJoint(definetrajleft);
                // // 提示用户安装新的牙刷
            //     char answers = 'n';
            //     while (answers != 'y')
            //     {
    ...
    int python_result = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\hand_eye_calibration\\GeneratePathOffset6.py\"");
    if (python_result != 0)
    {
    ...
    // 自动执行python代码获取旋转对齐基坐标的点云
    std::cout << "Running Python script to generate brush offsets..." << std::endl;
    int python_result1 = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\hand_eye_calibration\\GeneratePathOffset61.py\"");
    if (python_result1 != 0)
    {
        std::cerr << "Python script execution failed!" << std::endl;
        return -1;
    ...
    c2::Speed brushspeed{60, 60, 60};
    c2::Acc brushacc{30, 30, 30};
    // c2::CPos pose;
    ...
    std::ifstream infile("E:\\PlanX\\brush_offset.txt");
    if (!infile.is_open())
    {
    ...
    {
        std::cout << "Read values: " << dx << " >> " << dy << " >> " << dz << std::endl;
        brushpointsoffset.push_back({dx, dy, dz, -179.534, -0.215, 90.558});
    }
    ...
    if (!brushpoints.empty() && !brushpointsoffset.empty())
    {
        for (const auto &offset : brushpointsoffset)
        {
            c2::Point cpos;
            cpos.type = c2::PointType::Cart;
            cpos.cpos.x = brushpoints[0].x + offset.x;
            cpos.cpos.y = brushpoints[0].y + offset.y;
            cpos.cpos.z = brushpoints[0].z + offset.z;
            cpos.cpos.a = brushpoints[0].a;
            cpos.cpos.b = brushpoints[0].b;
            cpos.cpos.c = brushpoints[0].c;
            brushingtrajectory.AddMovJ(cpos, brushspeed, brushacc, c2::ZoneType::Fine, zone);
        }
    }

    if (!brushpoints.empty() && !brushpointsoffsetinverse.empty())
    {
        for (const auto &offset : brushpointsoffsetinverse)
        {
            c2::Point cpos;
            cpos.type = c2::PointType::Cart;
            cpos.cpos.x = brushpoints[0].x + offset.x;
            ...
            cpos.cpos.b = brushpoints[0].b;
            cpos.cpos.c = brushpoints[0].c;
            brushingtrajectoryinverse.AddMovJ(cpos, brushspeed, brushacc, c2::ZoneType::Fine, zone);
        }
    }
    ...
    while (answerbrush != 'n')
    {
        std::cout << "Brush again" << std::endl;
        std::cin >> answerbrush;
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectory);
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectoryinverse);
    }

    // 提示右侧刷牙
    char answersright = 'n';
    while (answersright != 'y')
    {
        std::cout << "We will brush right,please rotate to right angle and press y" << std::endl;
        std::cin >> answersright;
    }
    camera.captureAndSave(10);

    std::cout << "Running Python script to generate brush offsets..." << std::endl;
    int python_resultright = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\hand_eye_calibration\\GeneratePathOffset61.py\"");
    if (python_resultright != 0)
    {
        std::cerr << "Python script execution failed!" << std::endl;
        return -1;
    }

    c2::Speed brushspeedright{60, 60, 60};
    c2::Acc brushaccright{30, 30, 30};
    // c2::CPos pose;

    // 从拍摄的牙齿和牙刷位置开始作为刷牙起始点
    std::vector<PointData> brushpointsright;
    brushpointsright = {{25.411, 452.19, 422.686, -179.534, -0.215, 90.558}};

    // 读取从python生成的轨迹点
    std::vector<PointData> brushpointsoffsetright;

    std::ifstream infileright("E:\\PlanX\\brush_offset.txt");
    if (!infileright.is_open())
    {
        std::cerr << "Failed to open brush_offset.txt (right)" << std::endl;
        return -1;
    }else{
        std::cout<< "open brush_offset.txt (right)" << std::endl;
    }

    double dxright, dyright, dzright;
    while (infileright >> dxright >> dyright >> dzright)
    {
        std::cout << "Read values: " << dxright << " >> " << dyright << " >> " << dzright << std::endl;
        brushpointsoffsetright.push_back({dxright, dyright, dzright, -179.534, -0.215, 90.558});
    }

    infileright.close();

    // 翻转
    std::vector<decltype(brushpointsoffsetright)::value_type> brushpointsoffsetinverseright;
    for (auto it = brushpointsoffsetright.rbegin(); it != brushpointsoffsetright.rend(); ++it)
    {
        brushpointsoffsetinverseright.push_back(*it);
    }

    c2::MovJointSegments brushingtrajectoryright;
    c2::MovJointSegments brushingtrajectoryinverseright;

    // 添加起点
    if (!brushpointsright.empty())
    {
        c2::Point cposbrushright;
        cposbrushright.type = c2::PointType::Cart;
        cposbrushright.cpos.x = brushpointsright[0].x;
        cposbrushright.cpos.y = brushpointsright[0].y;
        cposbrushright.cpos.z = brushpointsright[0].z;
        cposbrushright.cpos.a = brushpointsright[0].a;
        cposbrushright.cpos.b = brushpointsright[0].b;
        cposbrushright.cpos.c = brushpointsright[0].c;
        brushingtrajectoryright.AddMovJ(cposbrushright, brushspeedright, brushaccright, c2::ZoneType::Fine, zone);
    }

    // 添加后续点（points前三位 + pointsoffset对应值，后三位保持与points相同）（后续需要修改python代码实现）
    if (!brushpointsright.empty() && !brushpointsoffsetright.empty())
    {
        for (const auto &offsetright : brushpointsoffsetright)
        {
            c2::Point cpos;
            cpos.type = c2::PointType::Cart;
            cpos.cpos.x = brushpointsright[0].x + offsetright.x;
            cpos.cpos.y = brushpointsright[0].y + offsetright.y;
            cpos.cpos.z = brushpointsright[0].z + offsetright.z;
            cpos.cpos.a = brushpointsright[0].a;
            cpos.cpos.b = brushpointsright[0].b;
            cpos.cpos.c = brushpointsright[0].c;
            brushingtrajectoryright.AddMovJ(cpos, brushspeed, brushacc, c2::ZoneType::Fine, zone);
        }
    }

    if (!brushpointsright.empty() && !brushpointsoffsetinverseright.empty())
    {
        for (const auto &offsetright : brushpointsoffsetinverseright)
        {
            c2::Point cposright;
            cposright.type = c2::PointType::Cart;
            cposright.cpos.x = brushpointsright[0].x + offsetright.x;
            cposright.cpos.y = brushpointsright[0].y + offsetright.y;
            cposright.cpos.z = brushpointsright[0].z + offsetright.z;
            cposright.cpos.a = brushpointsright[0].a;
            cposright.cpos.b = brushpointsright[0].b;
            cposright.cpos.c = brushpointsright[0].c;
            brushingtrajectoryinverseright.AddMovJ(cposright, brushspeedright, brushaccright, c2::ZoneType::Fine, zone);
        }
    }

    char answerbrushright = 'y';
    while (answerbrushright != 'n')
    {
        std::cout << "Brush again" << std::endl;
        std::cin >> answerbrushright;
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectoryright);
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectoryinverseright);
    }



    // 左侧刷牙
    char answersrights = 'n';
    while (answersrights != 'y')
    {
        std::cout << "We will brush right,please rotate to right angle and press y" << std::endl;
        std::cin >> answersrights;
    }
    camera.captureAndSave(10);

    std::cout << "Running Python script to generate brush offsets..." << std::endl;
    int python_resultrights = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\hand_eye_calibration\\GeneratePathOffset61.py\"");
    if (python_resultrights != 0)
    {
        std::cerr << "Python script execution failed!" << std::endl;
        return -1;
    }

    c2::Speed brushspeedrights{60, 60, 60};
    c2::Acc brushaccrights{30, 30, 30};
    // c2::CPos pose;

    // 从拍摄的牙齿和牙刷位置开始作为刷牙起始点
    std::vector<PointData> brushpointsrights;
    brushpointsrights = {{25.411, 452.19, 422.686, -179.534, -0.215, 90.558}};

    // 读取从python生成的轨迹点
    std::vector<PointData> brushpointsoffsetrights;

    std::ifstream infilerights("E:\\PlanX\\brush_offset.txt");
    if (!infilerights.is_open())
    {
        std::cerr << "Failed to open brush_offset.txt (right)" << std::endl;
        return -1;
    }else{
        std::cout<< "open brush_offset.txt (rights)" << std::endl;
    }

    double dxrights, dyrights, dzrights;
    while (infilerights >> dxrights >> dyrights >> dzrights)
    {
        std::cout << "Read values: " << dxrights << " >> " << dyrights << " >> " << dzrights << std::endl;
        brushpointsoffsetrights.push_back({dxrights, dyrights, dzrights, -179.534, -0.215, 90.558});
    }

    infilerights.close();

    // 翻转
    std::vector<decltype(brushpointsoffsetrights)::value_type> brushpointsoffsetinverserights;
    for (auto it = brushpointsoffsetrights.rbegin(); it != brushpointsoffsetrights.rend(); ++it)
    {
        brushpointsoffsetinverserights.push_back(*it);
    }

    c2::MovJointSegments brushingtrajectoryrights;
    c2::MovJointSegments brushingtrajectoryinverserights;

    // 添加起点
    if (!brushpointsrights.empty())
    {
        c2::Point cposbrushrights;
        cposbrushrights.type = c2::PointType::Cart;
        cposbrushrights.cpos.x = brushpointsrights[0].x;
        cposbrushrights.cpos.y = brushpointsrights[0].y;
        cposbrushrights.cpos.z = brushpointsrights[0].z;
        cposbrushrights.cpos.a = brushpointsrights[0].a;
        cposbrushrights.cpos.b = brushpointsrights[0].b;
        cposbrushrights.cpos.c = brushpointsrights[0].c;
        brushingtrajectoryrights.AddMovJ(cposbrushrights, brushspeedrights, brushaccrights, c2::ZoneType::Fine, zone);
    }

    // 添加后续点（points前三位 + pointsoffset对应值，后三位保持与points相同）（后续需要修改python代码实现）
    if (!brushpointsrights.empty() && !brushpointsoffsetrights.empty())
    {
        for (const auto &offsetrights : brushpointsoffsetrights)
        {
            c2::Point cpos;
            cpos.type = c2::PointType::Cart;
            cpos.cpos.x = brushpointsrights[0].x + offsetrights.x;
            cpos.cpos.y = brushpointsrights[0].y + offsetrights.y;
            cpos.cpos.z = brushpointsrights[0].z + offsetrights.z;
            cpos.cpos.a = brushpointsrights[0].a;
            cpos.cpos.b = brushpointsrights[0].b;
            cpos.cpos.c = brushpointsrights[0].c;
            brushingtrajectoryrights.AddMovJ(cpos, brushspeed, brushacc, c2::ZoneType::Fine, zone);
        }
    }

    if (!brushpointsrights.empty() && !brushpointsoffsetinverserights.empty())
    {
        for (const auto &offsetrights : brushpointsoffsetinverserights)
        {
            c2::Point cposrights;
            cposrights.type = c2::PointType::Cart;
            cposrights.cpos.x = brushpointsrights[0].x + offsetrights.x;
            cposrights.cpos.y = brushpointsrights[0].y + offsetrights.y;
            cposrights.cpos.z = brushpointsrights[0].z + offsetrights.z;
            cposrights.cpos.a = brushpointsrights[0].a;
            cposrights.cpos.b = brushpointsrights[0].b;
            cposrights.cpos.c = brushpointsrights[0].c;
            brushingtrajectoryinverserights.AddMovJ(cposrights, brushspeedrights, brushaccrights, c2::ZoneType::Fine, zone);
        }
    }

    char answerbrushrights = 'y';
    while (answerbrushrights != 'n')
    {
        std::cout << "Brush again" << std::endl;
        std::cin >> answerbrushrights;
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectoryrights);
        arm.StartBrushTeethTrajectoryJoint(brushingtrajectoryinverserights);
    }


    // 退回到静态位置（需要根据实际位置编写逻辑，后续补充）
    std::vector<PointData> pointsinit = {
        {226.063, 474.071, 409.245, -179.907, 1.355, -6.226},
    };

    for (const auto &pt : pointsinit)
    {
        c2::MovJointSegments definetrajinit;
        c2::Point cpos1init;
        cpos1init.type = c2::PointType::Cart;
        cpos1init.cpos.x = pt.x;
        cpos1init.cpos.y = pt.y;
        cpos1init.cpos.z = pt.z;
        cpos1init.cpos.a = pt.a;
        cpos1init.cpos.b = pt.b;
        cpos1init.cpos.c = pt.c;
        definetrajinit.AddMovJ(cpos1init, speed, acc, c2::ZoneType::Fine, zone);
        arm.StartBrushTeethTrajectoryJoint(definetrajinit);
    }

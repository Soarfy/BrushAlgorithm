...
int main()
{
    ...
    for (const auto &pt : points)
    {
        ...
        else if (count == 17)
        {
            ...
            while (answers != 'y')
            {
                ...
                std::cin >> answers;
                // 旋转牙刷
                int rotateleft = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\TcpclientRight.py\"");
            }
        ...
        else if (count == 18)
        {
            ...
            while (answers != 'y')
            {
                ...
                std::cin >> answers;
                // 旋转牙刷
                int rotateleft = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\Tcpclient.py\"");
            }
            camera.captureAndSave(imgcount);
        }
        else
    ...
    int python_result = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\GeneratePathOffset66initCenter.py\"");
    if (python_result != 0)
    {
    ...
    // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@自动执行python代码获取旋转对齐基坐标的点云，新的算法可以直接将牙刷模型导入了，生成轨迹部分@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    std::cout << "Running Python script to generate brush offsets..." << std::endl;
    // int python_result1 = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\GeneratePathOffset66.py\"");
    // std::this_thread::sleep_for(std::chrono::seconds(2));

    int python_result1 = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\GeneratePathOffset66BrushCenter.py\"");
    std::this_thread::sleep_for(std::chrono::seconds(2));

    if (python_result1 != 0)
    {
        std::cerr << "Python script GeneratePathOffset66Brush.py execution failed!" << std::endl;
        return -1;
    }

    // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@生成的轨迹转移到机械臂末端@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    int python_result11 = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\TCPRotation10Center.py\"");

    if (python_result11 != 0)
    {
        std::cerr << "Python script TCPRotation10.py execution failed!" << std::endl;
        return -1;
    }
    ...
    if (!brushpointsoffset_ee_poses.empty())
    {
        ...
        for (const auto &offset : brushpointsoffset_ee_poses)
        {
            ...
            countvalue += 1;
            // char answers = 'n';
            // std::cout << "move next endpose" << std::endl;
            // std::cin >> answers;
        }
    }

    // // 根据拍摄的MarKer来计算出需要弥补的轨迹
    int python_result111 = std::system("cmd /c \"D:\\UsmileProject\\hand_eye_calibration\\.venv312\\Scripts\\activate && python D:\\UsmileProject\\hand_eye_calibration\\AutoDetectDiff6.py\"");
    std::this_thread::sleep_for(std::chrono::seconds(2));
    ...
    // std::ifstream brushvectorinfile("C:\\Users\\pengjm\\Desktop\\Release\\brushing_vectors.txt");
    ...
    while (brushvectorinfile >> dxbrush >> dybrush >> dzbrush)
    {
        ...
        // printf("dxbrush: %.2f dybrush: %.2f dzbrush: %.2f \n",
        //            dxbrush, dybrush, dzbrush);
    }
    brushvectorinfile.close();

    // 根据期望的力道来调整轨迹
    double targetforcevalue = 0.2;
    std::ofstream forcerepairedoutputfile("D:\\UsmileProject\\hand_eye_calibration\\transformsrepairedforce.txt");
    // std::ofstream forcerepairedoutputfile("C:\\Users\\pengjm\\Desktop\\Release\\transformsrepairedforce.txt");
    ...
    for (size_t i = 0; i < brushpointsoffsetrepair.size(); ++i)
    {
        ...
        while (!converged)
        {
            ...
            // {
            //     std::cerr << "Successed to get force data\n";
            //     printf("fx: %.2f fy: %.2f fz: %.2f mx: %.2f my: %.2f mz: %.2f\n",
            //            force[0], force[1], force[2], force[3], force[4], force[5]);
            // }
            while (obj->GetCurrentForceData(force) != 28)
            {
                std::cerr << "failed to get force data\n";
            }

            // 对force清零
            // if (obj->GetCurrentForceData(force) == 28 && firstcount == 0)
            // {
            //     forcefirst[0] = force[0];
            //     forcefirst[1] = force[1];
            //     forcefirst[2] = force[2];
            //     firstcount = 1;
            //     printf("forcefirst0: %.2f forcefirst1: %.2f forcefirst2: %.2f \n",
            //            forcefirst[0], forcefirst[1], forcefirst[2]);
            //     char answers = 'n';
            ...
            // 把误差转回到 XYZ 修正量(每个方向的值还可以不一样现在)
            // Eigen::Vector3d delta = -2 * err * brushDir;
            // Eigen::Vector3d delta = 0.02  * brushDir;
    ...
    // 检测是否关掉了6维力
    char answers = 'n';
    std::cout << "Test 6 force y" << std::endl;
    std::cin >> answers;

    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@加载修复好固定力控的数据@@@@@@@@@@@@@@@@@@@@@@@2
    std::ifstream forceinfilerepair("D:\\UsmileProject\\hand_eye_calibration\\transformsrepairedforce.txt");

    // std::ifstream forceinfilerepair("D:\\UsmileProject\\hand_eye_calibration\\transformsrepaired.txt");
    ...
    {
        size_t start_index = group * 4;
        size_t end_index = start_index + 4;

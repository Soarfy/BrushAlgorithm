...
public:
    ...
    // 拟合指定轨迹
    // 1) 通过字符串 PointList（点列表或轨迹文件名）
    std::string MovS(std::string pointList);
    template <typename... Args>
    std::string MovS(std::string pointList, Args... args);
    // 1a) 通过字符串 PointList + MovSParams 参数结构体
    std::string MovS(std::string pointList, const MovSParams& params);
    // 2) 通过关节点位列表，内部会转换为 PointList 字符串
    std::string MovS(const std::vector<CJointPoint>& points);
    template <typename... Args>
    std::string MovS(const std::vector<CJointPoint>& points, Args... args);
    // 2a) 通过关节点位列表 + MovSParams 参数结构体
    std::string MovS(const std::vector<CJointPoint>& points, const MovSParams& params);
    // 3) 通过笛卡尔点位列表，内部会转换为 PointList 字符串
    std::string MovS(const std::vector<CDescartesPoint>& points);
    template <typename... Args>
    std::string MovS(const std::vector<CDescartesPoint>& points, Args... args);
    // 3a) 通过笛卡尔点位列表 + MovSParams 参数结构体
    std::string MovS(const std::vector<CDescartesPoint>& points, const MovSParams& params);
    ...
    std::string RelMovJTool( CDescartesPoint& pt,int user=0,int tool=0,int a=10,int v=10,int cp=100);
...
template <typename... Args>
std::string CDashboard::MovS(std::string pointList, Args... args)
{
    std::unique_lock<std::mutex> lockValue(m_mutexSend);
    std::string str = "MovS(" + std::string(pointList) + ",";
    strSend = str;
    printArg(args...);
    str = strSend;
    strSend.clear();
...
template <typename... Args>
std::string CDashboard::MovS(const std::vector<CJointPoint>& points, Args... args)
{
    // 将关节点位列表转换为 PointList 字符串，例如 {{j1,...,j6},{j1,...,j6},...}
    std::ostringstream oss;
    oss << "{";
    ...
    oss << "}";
    return MovS(oss.str(), args...);
}

template <typename... Args>
std::string CDashboard::MovS(const std::vector<CDescartesPoint>& points, Args... args)
{
    // 将笛卡尔点位列表转换为 PointList 字符串，例如 {{x,y,z,rx,ry,rz},{x,y,z,rx,ry,rz},...}
    std::ostringstream oss;
    oss << "{";
    for (size_t i = 0; i < points.size(); ++i) {
        oss << "{" << points[i].ToString() << "}";

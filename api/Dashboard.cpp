...
{
    std::string str = "GetPose( user = " + std::to_string(user) + ", tool = " + std::to_string(tool) + ")";
...
std::string CDashboard::MovS(std::string pointList)
{
    // MovS(PointList,{tool=0,user=0,v=100,speed=100,a=100,freq=1,stopcond=""})
    // 此处仅负责拼接指令字符串并发送，PointList 可以是点列表 {P1,P2,...} 或轨迹文件名 "demo.csv"
    std::string str = "MovS(" + std::string(pointList) + ")";

    return SendRecvMsg(str);
}

std::string CDashboard::MovS(std::string pointList, const MovSParams& params)
{
    // MovS(PointList,{tool=0,user=0,v=100,speed=100,a=100,freq=1,stopcond=""})
    std::string str = "MovS(" + std::string(pointList) + "," + params.ToString() + ")";

    return SendRecvMsg(str);
}

std::string CDashboard::MovS(const std::vector<CJointPoint>& points)
{
    // 将关节点位列表转换为 PointList 字符串，然后复用 MovS(std::string)
    std::ostringstream oss;
    oss << "{";
...
std::string CDashboard::MovS(const std::vector<CJointPoint>& points, const MovSParams& params)
{
    // 将关节点位列表转换为 PointList 字符串，然后复用 MovS(std::string, MovSParams)
    std::ostringstream oss;
    oss << "{";
    for (size_t i = 0; i < points.size(); ++i) {
        oss << "{" << points[i].ToString() << "}";
    ...
    oss << "}";
    return MovS(oss.str(), params);
}

std::string CDashboard::MovS(const std::vector<CDescartesPoint>& points)
{
    // 将笛卡尔点位列表转换为 PointList 字符串，然后复用 MovS(std::string)
    std::ostringstream oss;
    oss << "{";
    for (size_t i = 0; i < points.size(); ++i) {
        oss << "{" << points[i].ToString() << "}";
...
std::string CDashboard::MovS(const std::vector<CDescartesPoint>& points, const MovSParams& params)
{
    // 将笛卡尔点位列表转换为 PointList 字符串，然后复用 MovS(std::string, MovSParams)
    std::ostringstream oss;
    //oss << "{";
    for (size_t i = 0; i < points.size(); ++i) {
        oss << "pose={" << points[i].ToString() << "}";
        if (i + 1 < points.size()) {
            oss << ",";
        }
    }
...
std::string CDashboard::RelMovJTool( CDescartesPoint& pt,int user,int tool,int a,int v,int cp)
{
    std::ostringstream oss;
    oss << "RelMovJTool("
        << pt.x << ","   // offsetX (mm)

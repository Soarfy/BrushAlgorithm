...
int main()
{
    ...
    int python_result2 =
        std::system("cmd /c "
                    ...
                    "venv312\\Scripts\\activate && python "
                    "D:\\UsmileProject\\hand_eye_"
                    "calibration\\GeneratePathOffset66BrushOther6dorobots.py\"");
    std::this_thread::sleep_for(std::chrono::seconds(2));
    ...
    //       demo->RelMovJDemo(pc1,0,3,20,50,100);

    //       std::cout << "move next tool step, please press Enter" << std::endl;
    //       std::cin.ignore((std::numeric_limits<std::streamsize>::max)(), '\n'); // 等待用户按下Enter

    //     }

    // Dobot::CDescartesPoint prev_point; // 用于存储前一个点
    ...
    //     {
    //         // 后续点：计算相对于前一个点的差值
    //         pc1.x = offset.x - prev_point.x;
    //         pc1.y = offset.y - prev_point.y;
    //         pc1.z = offset.z - prev_point.z;
    //         pc1.rx = offset.a - prev_point.rx;
    //         pc1.ry = offset.b - prev_point.ry;
    //         pc1.rz = offset.c - prev_point.rz;

    //         // 更新前一个点为当前原始点
    //         prev_point.x = offset.x;
    //         prev_point.y = offset.y;
    //         prev_point.z = offset.z;
    //         prev_point.rx = offset.a;
    //         prev_point.ry = offset.b;
    //         prev_point.rz = offset.c;
    //     }
    ...
    for (const auto &offset : brushpointsoffset_ee_poses)
    {
        ...
        if (first_point)
        {
            // 第一个点：直接使用原始坐标，并保留两位小数
            pc1.x = std::round(offset.x * 100.0) / 100.0;
            pc1.y = std::round(offset.y * 100.0) / 100.0;
            pc1.z = std::round(offset.z * 100.0) / 100.0;
            pc1.rx = std::round(offset.a * 100.0) / 100.0;
            pc1.ry = std::round(offset.b * 100.0) / 100.0;
            pc1.rz = std::round(offset.c * 100.0) / 100.0;
        ...
        {
            // 后续点：计算相对于前一个点的差值，并保留两位小数
            pc1.x = std::round((offset.x - prev_point.x) * 100.0) / 100.0;
            pc1.y = std::round((offset.y - prev_point.y) * 100.0) / 100.0;
            pc1.z = std::round((offset.z - prev_point.z) * 100.0) / 100.0;
            pc1.rx = std::round((offset.a - prev_point.rx) * 100.0) / 100.0;
            pc1.ry = std::round((offset.b - prev_point.ry) * 100.0) / 100.0;
            pc1.rz = std::round((offset.c - prev_point.rz) * 100.0) / 100.0;

            // 更新前一个点为当前原始点，也保留两位小数
            prev_point.x = std::round(offset.x * 100.0) / 100.0;
            prev_point.y = std::round(offset.y * 100.0) / 100.0;
            prev_point.z = std::round(offset.z * 100.0) / 100.0;
            prev_point.rx = std::round(offset.a * 100.0) / 100.0;
            prev_point.ry = std::round(offset.b * 100.0) / 100.0;
            prev_point.rz = std::round(offset.c * 100.0) / 100.0;
        }

...
int main()
{
    ...
    for (const auto &pt : points)
    {
        ...
        {
            camera.captureAndSave(imgcount); // 保存为1001.bmp和1001.ply
            imgcount += 1;
            count += 1;
    ...
    int python_result = std::system("cmd /c \"D:\\line\\pythonProject\\.venv\\Scripts\\activate && python D:\\hand_eye_calibration\\GeneratePathOffsetold.py\"");
    if (python_result != 0)
    {
    ...
    // 提示用户移动到起始点并等待确认
    std::cout << "Please move brush to the start point, when you finished it please press Enter" << std::endl;
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // 等待用户按下Enter
    // 切回自动模式
    std::cout << "changeRobotMode: " << arm.changeRobotMode(c2::UserCommand::ToAuto) << std::endl;

    // 挪好后获取位姿作为起始点
    c2::Speed brushspeed{10, 10, 10};
    ...
    if (cart_ok) // 只有获取位姿成功时才添加到points
    {
        brushpoints.push_back({pose.x, pose.y, pose.z, pose.a, pose.b, pose.c});
        std::cout << " (brush point x=" << brushpoints[0].x << ", y=" << brushpoints[0].y << ", z=" << brushpoints[0].z
                  << ", a=" << brushpoints[0].a << ", b=" << brushpoints[0].b << ", c=" << brushpoints[0].c << ")";
    }
    else // 如果获取失败，使用默认值
    {
        brushpoints = {
            {30.129, 461.646, 334.887, 178.871, 3.32, -2.227}};
    }

    // std::vector<PointData> brushpointsoffset = {
    //     {0.37564096, 3.01542806, 0.73420392},
    //     {0.64719455, 8.1938763, 1.75920241},
    //     {0.63446767, 12.04292945, 2.01836133},
    //     {0.78360591, 15.08380296, 1.13794173},
    //     {1.16006635, 22.75969159, -1.08444692},
    //     {0.78360591, 15.08380296, 1.13794173},
    //     {0.63446767, 12.04292945, 2.01836133},
    //     {0.64719455, 8.1938763, 1.75920241},
    //     {0.37564096, 3.01542806, 0.73420392}};
    ...
    while (infile >> dx >> dy >> dz)
    {
        ...
        brushpointsoffset.push_back({dx, dy, dz, 178.871, 3.32, -2.227});
    }
    ...
    // 添加后续点（points前三位 + pointsoffset对应值，后三位保持与points相同）

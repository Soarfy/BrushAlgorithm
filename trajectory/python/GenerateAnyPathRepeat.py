...
class BrushBuilderGUI:
    ...
    def __init__(self, points, ply_path=None):
        self.points = points
        self.n = len(points)
    ...
    def save_all_segments(self):
        save_path = "D:\\UsmileProject\\hand_eye_calibration\\all_segments.txt"
...
def rotate_point_cloud_with_selected_axis(ply_path, save_prefix="rotated"):
    ...
    3. Ask user for rotation command:
         1 → +45°
         2 → -45°
         3 → no rotation
...
class BrushPathProcessor:
    ...
    def save_pointcloud_along_path_as_plyv21(self, config_index_values,pcd, path_pts, mesh_direction_pts,
                                             output_folder="saved_ply"):
        """
        mesh_direction_pts: 旋转轴和中心的三个点坐标
        mesh_direction_ids: 对应点在点云中的索引，用于配准
        ...
        try:
            with open(transform_file, "w") as f:
                for i, (pos, dir_vec) in enumerate(zip(path_pts, directions)):
                    ...
                    # 欧拉角（度）
                    rz, ry, rx = R.from_matrix(R_local).as_euler("zyx", degrees=True)

                    rx = 0
                    ry = 0
                    if(config_index_values==6):
                        rx = 0
                        ry = 0
                        rz = 0

                    if (i == 0 and config_index_values != 7 and config_index_values!=6):
                        if(config_index_values == 0 or config_index_values ==3 or config_index_values ==4):
                            rz_first = rz + 5
                            rz = -5
                        else:
                            rz_first = rz - 5
...
# 第一步，用于生成理论轨迹值和理论轨迹向量，需要添加拟合的函数
# if __name__ == "__main__":
#     pcd2 = "D:/UsmileProject/hand_eye_calibration/mergeredoldboard.ply"
#     pcd2 = o3d.io.read_point_cloud(pcd2)
#     selectedbrush2 = o3d.io.read_point_cloud("D:/UsmileProject/hand_eye_calibration/Initmergeredbrush.ply")
#
#     processor = BrushPathProcessor()
#     picked_path = np.load(r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightup\picked_path.npy")
#     mesh_direction_pts = np.load(r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightup\mesh_direction_pts.npy")
#
#     transformArray = processor.save_pointcloud_along_path_as_plyv21(selectedbrush2, picked_path, mesh_direction_pts,
#
#                                         output_folder="saved_ply")
...
from trajectory_paths import DEFAULTCONFIG_DIR, HAND_EYE_DATA_DIR
    ...
    path_configs = {
        0: {  # rightup
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightup\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightup\mesh_direction_pts.npy"
        },
        1: {  # leftup
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftup\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftup\mesh_direction_pts.npy"
        },
        2: {  # rightside
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightside\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightside\mesh_direction_pts.npy"
        },
        3: {  # leftside
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftside\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftside\mesh_direction_pts.npy"
        },
        4: {  # rightinside
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightinside\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\rightinside\mesh_direction_pts.npy"
        },
        5: {  # leftinside
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftinside\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\leftinside\mesh_direction_pts.npy"
        },
        6: {  # center
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\center\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\center\mesh_direction_pts.npy"
        },
        7: {  # center
            "picked_path": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\sideahead\picked_path.npy",
            "mesh_direction_pts": r"D:\UsmileProject\hand_eye_calibration\UsmileUi\defaultconfig\sideahead\mesh_direction_pts.npy"
        }
    }
    ...
    if len(sys.argv) > 1:
        try:
            ...
            subfolder = get_subfolder_by_param(config_index)
        except ValueError:
            print("错误：参数必须是数字 0-6")
            sys.exit(1)
    else:
        print("请选择配置：")
        print("0: rightup")
        ...
        try:
            config_index = int(input("请输入数字 (0-7): "))
        except ValueError:
            print("错误：请输入有效的数字")
            sys.exit(1)

    # 验证输入
    if config_index not in path_configs:
        print(f"错误：无效的配置索引 {config_index}，请输入 0-7 之间的数字")
        sys.exit(1)

    # 获取对应的配置
    config = path_configs[config_index]
    config_names = ["rightup", "leftup", "rightside", "leftside", "rightinside", "leftinside", "center","sideahead"]
    selected_config = config_names[config_index]

    print(f"正在处理配置: {selected_config}")
    ...
    # 使用选中的配置路径
    picked_path = np.load(config["picked_path"])
    mesh_direction_pts = np.load(config["mesh_direction_pts"])

    save_dir = f"{DEFAULTCONFIG_DIR}/{subfolder}/"

    config_index_value = int(sys.argv[1])
    transformArray = processor.save_pointcloud_along_path_as_plyv21(
        config_index_value,
        selectedbrush2,
        picked_path,
        mesh_direction_pts,

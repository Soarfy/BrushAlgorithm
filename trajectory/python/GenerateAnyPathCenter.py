...
def rotate_point_cloud_with_selected_axis(ply_path, save_prefix="rotated", rotation_param=None):
    ...
    if rotation_param is not None:
        ...
        elif rotation_param == 2 or rotation_param == 5 or rotation_param == 6:
...
if __name__ == "__main__":
    ...
    selectedbrush2 = "D:/UsmileProject/hand_eye_calibration/Initmergeredbrush.ply"
    selectedbrush2 = o3d.io.read_point_cloud(selectedbrush2)
    processor = BrushPathProcessor()

    picked_path, picked_ids = processor.pick_vertex_indices_from_pointcloud(pcd2)
    picked_path = processor.smooth_and_sample_path(picked_path, smooth_factor=0.6, num_samples=18)
    ...
    # 根據參數創建不同的保存目錄
    save_dir = f"{DEFAULTCONFIG_DIR}/center/"
    ...
    app = BrushBuilderGUI(picked_path, ply_path_we,'center')
    app.run()

    # center 区域不需要选择浮刷点(support_points)，跳过该步骤(其轨迹流程不依赖 support_points.txt)
    # ply_path_we = "D:/UsmileProject/hand_eye_calibration/mergeredoldboard.ply"
    # app2 = SupportPointSelector(picked_path, ply_path_we, "center")
    # app2.run()
    print("[INFO] center 无需选择浮刷点(support_points)，已跳过。")

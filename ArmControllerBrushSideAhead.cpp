...
namespace
{
bool sideAheadMoveRobotJoint(const Dobot::CJointPoint &pt)
{
    static Dobot::CDashboard jointDashboard;
    static bool jointDashboardConnected = false;
    if (!jointDashboardConnected)
    {
        jointDashboard.Connect("192.168.5.1", 29999);
        jointDashboardConnected = true;
    }

    jointDashboard.EnableRobot();
    const std::string result = jointDashboard.MovJ(pt);
    std::cout << "MovJ(joint) reply: " << result << std::endl;

    std::regex numberRe("-?[0-9]+");
    std::vector<int> nums;
    for (std::sregex_iterator it(result.begin(), result.end(), numberRe), end; it != end; ++it)
    {
        nums.push_back(std::stoi(it->str()));
    }
    if (nums.empty() || nums[0] != 0)
    {
        std::cerr << "MovJ(joint) 下发失败" << std::endl;
        return false;
    }
...
int main()
{
    ...
    // 压力值为0时用48°工具回转，否则用65°
    rotatetooljoint.ry = (pressureParameter == 0.0) ? -48.0 : -65.0;
    std::cout << "rotatetooljoint.ry: " << rotatetooljoint.ry << std::endl;
    ...
    auto loadPickedPathPoints = [&]() -> bool {
        ...
        pickedPathPoints.clear();
        double pdx, pdy, pdz, prx, pry, prz;
        while (picked_path_infile >> pdx >> pdy >> pdz >> prx >> pry >> prz)
        {
    ...
    auto runPickedPathToFrontTeeth = [&](const std::string &stageTag,
                                          const Dobot::CDescartesPoint *trajStartAfterGuide = nullptr)
    {
        ...
        {
            Dobot::CDescartesPoint guideExitJoint{};
            guideExitJoint.x = 58.8848;
            guideExitJoint.y = -393.7958;
            guideExitJoint.z = 528.6063;
            guideExitJoint.rx = -179.6860;
            guideExitJoint.ry = 43.6490;
            guideExitJoint.rz = -160.6180;

            demo->moveRobotC(guideExitJoint, guideExitJoint);
            std::cout << "[" << stageTag << "] 前导轨迹末点完成，MovJ 前往固定关节过渡位..." << std::endl;
            // if (!sideAheadMoveRobotJoint(guideExitJoint))
            // {
            //     std::cerr << "[" << stageTag << "] 固定关节过渡位运动失败" << std::endl;
            //     return;
            // }
            std::cout << "[" << stageTag << "] 过渡完成，自动前往刷牙轨迹起点..." << std::endl;
            demo->moveRobotC(*trajStartAfterGuide, *trajStartAfterGuide);
        }
    };

    auto liftRelativeMm = [&](double mm) {
    ...
    if (useExistingTrajectoryForForce)
    {
        ...
        if (!loadPickedPathPoints())
        {
            return -1;
        }

        std::cout << "先上抬 60mm..." << std::endl;
        Dobot::CDescartesPoint fastPathLift{};
        fastPathLift.x = 0;
        fastPathLift.y = 0;
        fastPathLift.z = -60;
        fastPathLift.rx = 0;
        fastPathLift.ry = 0;
        fastPathLift.rz = 0;
        demo->RelMovJDemo(fastPathLift, 0, 5, 20, 50, 100);

        std::cout << "从上抬位直接前往前牙引导轨迹..." << std::endl;
        ...
        runPickedPathToFrontTeeth("力控前", &pointstarts);
        ...
        std::cout << "前牙引导完成，按 Enter 开始力控..." << std::endl;
        std::cin.ignore((std::numeric_limits<std::streamsize>::max)(), '\n');
    }
    else
    {
    ...
    while (!userSatisfied)
    {
        ...
        std::cout << "\n=========== 新一轮轨迹调整开始 ===========\n";
        runPickedPathToFrontTeeth("轨迹微调前", &pointstart);
    ...
    {
        // ================= 模式1 / 模式3 精简自动流程 (保留前牙引导轨迹) =================
        const std::string Std_Traj_Json = "../defaultconfig/sideahead/standard_trajectory.json";
        const std::string Std_Traj_Json_NoComp = "../defaultconfig/sideahead/standard_trajectory_nocomp.json";
        ...
        // ---- 读取前牙引导轨迹(后续微调/力控/执行均需要) ----
        if (!loadPickedPathPoints())
        {
            return -1;
        }

        // ---- 轨迹微调循环(模式1/3都做, 经前牙引导轨迹接近) ----
        bool userSatisfied = false;
        while (!userSatisfied)
        {
            std::cout << "\n=========== 新一轮轨迹调整开始 ===========\n";
        ...
        pointstarts.rz = firstPoses.rz;
        runPickedPathToFrontTeeth("力控前", &pointstarts);
        std::cout << "[模式" << mode << "] 前牙引导完成, 即将开始力控..." << std::endl;

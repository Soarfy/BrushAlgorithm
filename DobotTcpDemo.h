...
public:
    ...
    void stopDrag();
private:
    bool waitQueuedCommandFinish(const std::string& recvData, const char* commandName, int timeoutMs);
    bool waitCommandIdFinish(int currentCommandID, int timeoutMs);
...
private:
    ...
    std::mutex m_mutexDashboard; // 串行化 m_Dashboard(控制端口)的请求-响应，避免多线程串包
